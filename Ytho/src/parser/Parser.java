...
public class Parser {
    ...
    public Parser(Scanner scan) {
        ...
        rules.put("r6", new ReductionRule(TokenType.STMNT, 2, "<Statement> := <Exp> ;"));
        rules.put("r7", new ReductionRule(TokenType.STMNT, 1, "<Statement> := <StmntBlk>"));
        rules.put("r8", new ReductionRule(TokenType.STMNT, 1, "<Statement> := <Cond>"));
        rules.put("r9", new ReductionRule(TokenType.STMNT, 1, "<Statement> := <Loop>"));
        rules.put("r10", new ReductionRule(TokenType.STMNT, 2, "<Statement> := <InOut> ;"));
        rules.put("r11", new ReductionRule(TokenType.STMNT, 2, "<Statement> := BASAG ;"));

        rules.put("r12", new ReductionRule(TokenType.DECLN, 2, "<Decln> := <DType> <Declr>  "));
        rules.put("r13", new ReductionRule(TokenType.DECLN, 4, "<Decln> := <DType> <Declr> ,<Declr> "));

        rules.put("r14", new ReductionRule(TokenType.DECLR, 1, "<Declr> := ID "));
        rules.put("r15", new ReductionRule(TokenType.DECLR, 3, "<Declr> := ID = <VarInit> "));

        rules.put("r16", new ReductionRule(TokenType.VARINIT, 1, "<VarInit> := <Exp> "));

        rules.put("r17", new ReductionRule(TokenType.EXP, 1, "<Exp> := <LogicExp> "));
        rules.put("r18", new ReductionRule(TokenType.EXP, 1, "<Exp> := <CastExp> "));
        rules.put("r19", new ReductionRule(TokenType.EXP, 1, "<Exp> := <LitExp> "));
        rules.put("r20", new ReductionRule(TokenType.EXP, 1, "<Exp> := ID "));

        rules.put("r21", new ReductionRule(TokenType.ARITHEXP, 3, "<ArithExp> := <ArithExp> ADDSUB <Term>"));
        rules.put("r22", new ReductionRule(TokenType.ARITHEXP, 1, "<ArithExp> := <Term> "));
        ...
        rules.put("r29", new ReductionRule(TokenType.LOGICEXP, 3, "<LogicExp> := <LogicExp> <Logop> <RelExp>"));
        rules.put("r30", new ReductionRule(TokenType.LOGICEXP, 1, "<LogicExp> := <RelExp>"));
        rules.put("r31", new ReductionRule(TokenType.LOGICEXP, 1, "<LogicExp> := BOOLCONST"));

        rules.put("r32", new ReductionRule(TokenType.LOGOP, 1, "<Logop> := AND"));
        rules.put("r33", new ReductionRule(TokenType.LOGOP, 1, "<Logop> := OR"));
        rules.put("r34", new ReductionRule(TokenType.LOGOP, 1, "<Logop> := NOT"));
        rules.put("r35", new ReductionRule(TokenType.LOGOP, 1, "<Logop> := NAND"));
        rules.put("r36", new ReductionRule(TokenType.LOGOP, 1, "<Logop> := NOR"));

        rules.put("r37", new ReductionRule(TokenType.RELEXP, 3, "<RelExp> := <RelExp> RELOP <ArithExp>"));
        rules.put("r38", new ReductionRule(TokenType.RELEXP, 1, "<RelExp> := <ArithExp>"));

        rules.put("r39", new ReductionRule(TokenType.CASTEXP, 4, "<CastExp> := ( <Dtype> ) <Exp>"));

        rules.put("r40", new ReductionRule(TokenType.LITEXP, 1, "<LitExp> := INTLIT"));
        rules.put("r41", new ReductionRule(TokenType.LITEXP, 1, "<LitExp> := STRINGLIT"));
        rules.put("r42", new ReductionRule(TokenType.LITEXP, 1, "<LitExp> := FLOATLIT"));

        rules.put("r43", new ReductionRule(TokenType.STMNT_BLK, 3, "<StmntBlk> := { <Stmnt> }"));

        rules.put("r44", new ReductionRule(TokenType.COND, 7, "<Cond> := KUNG ( <Exp> ) { <Stmnt> } "));
        rules.put("r45", new ReductionRule(TokenType.COND, 11, "<Cond> := KUNG ( <Exp> ) { <Stmnt> } MAIBA { <Stmnt> } "));

        rules.put("r46", new ReductionRule(TokenType.LOOP, 7, "<Loop> := PARAIKOT ( <LoopCond> ) { <Stmnts> } "));
 
        rules.put("r47", new ReductionRule(TokenType.LOOPCOND, 5, "<LoopCond> := < Decln > ; <Exp> ; <UpDown> "));
        ...
        rules.put("r54", new ReductionRule(TokenType.STRPAT, 3, "<StrPat> := LQUOTE STRINGLIT RQUOTE "));
        rules.put("r55", new ReductionRule(TokenType.STRPAT, 1, "<StrPat> := ID "));
        rules.put("r56", new ReductionRule(TokenType.STRPAT, 5, "<StrPat> := LPAREN STRINGLIT RPAREN CONCAT <StrPat> "));
